...
x = [i.split(' ')[0] for i in mylist]
x1 = [i.split('-')[0] for i in x]
x2 = [i.split('-')[1] for i in x]


y = [i.split(' ')[1] for i in mylist]
z = [i.split(' ')[2] for i in mylist]
...
df[2].replace(':','', regex=True, inplace=True)

count = []
def not_iter_row():
    for r, s in df.iterrows():
        if s[3].count(s[2]) <= int(s[1]) and s[3].count(s[2]) >= int(s[0]):
            count.append(r)
    print(len(count))
not_iter_row()